...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites):
        ...
        self.turn = 0
        # Undo button specifications and history
        self.undo_button_rect = pygame.Rect(900, 500, 200, 50)
        self.history = []
    ...
    def add_piece(self, row, col, player):
        if self.valid_move(row, col, player):
            # Saves game state after every turn
            self.save_state()
    ...
    # Save state is a new member function that will save the history of the current board
    def save_state(self):
        new_board = [[cell for cell in row] for row in self.board]
        self.history.append(new_board)

    # This function will undo the player's move
    def undo_move(self):
        if self.history:
...
class MainMenu:
    def __init__(self):
        self.difficulty_dropdown = Dropdown(400, 300, 200, 50, ['Easy', 'Medium', 'Hard'])
        self.difficulty_levels = [2, 4, 6]  # Adjust the difficulty levels as needed
        self.selected_difficulty = None
        self.start_button_rect = pygame.Rect(400, 400, 200, 50)
        self.start_button_pressed = False  # Flag to track button press

    def draw(self, window):
        font = pygame.font.Font(None, 36)
        text = font.render("Select Difficulty:", True, BLACK)
        window.blit(text, (300, 200))
        self.difficulty_dropdown.draw(window)

        pygame.draw.rect(window, BLACK, self.start_button_rect, 2)
        start_text = font.render("Start", True, BLACK)
        window.blit(start_text, (self.start_button_rect.x + 5, self.start_button_rect.y + 5))
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.start_button_rect.collidepoint(x, y):
                self.start_button_pressed = True  # Set the flag on button press

    def start_game(self):
        if self.start_button_pressed:
            self.selected_difficulty = self.difficulty_levels[self.difficulty_dropdown.get_choice()]
            # Add code to start the game with the selected difficulty
...
while in_main_menu:
    ...
    difficulty = main_menu.start_game()
...
# Sets bot difficulty
bots[0].set_difficulty(difficulty)
bots[1].set_difficulty(difficulty)
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        # Check if the mouse click is within the bounds of the "Undo Move" button
        elif event.type == pygame.MOUSEBUTTONDOWN:
            x, y = event.pos
            if board.undo_button_rect.collidepoint(x, y):
                board.undo_move()
            else:
    ...
    # Draws undo button
    pygame.draw.rect(window, BLACK, board.undo_button_rect, 2)
    undo_text = font.render("Undo Move", True, BLACK)
    window.blit(undo_text, (board.undo_button_rect.x + 5, board.undo_button_rect.y + 5))
